...
if __name__ == "__main__":  # pragma: no cover
    ...
    p.add_argument('azimuth', help='azimuth [deg]', nargs='?', type=float)
    p.add_argument('elevation', help='elevation [deg]', nargs='?', type=float)
    p.add_argument('lat', help='WGS84 obs. lat [deg]', nargs='?', type=float)
    p.add_argument('lon', help='WGS84 obs. lon [deg]', nargs='?', type=float)
    p.add_argument('time', help='obs. time YYYY-mm-ddTHH:MM:SSZ', nargs='?')
    p.add_argument('--idl', help='run Kitts Peak example IDL Astrolib',
                   action='store_true')
    a = p.parse_args()
